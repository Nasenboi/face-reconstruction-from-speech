...
def calculate_forward_degree(keypoints: dict, box: list) -> float:
    ...
    return (eye_center_distance + mouth_center_distance + nose_center_distance) / 3
...
parser.add_argument("-d", "--delete", default=False, help="Choose to delete the frame folder or not")
...
for database_id in frame_folder_iterator:
    frames_path = os.path.join(frame_folder, database_id)
    ...
    for f in frames:
        ...
        try:
            ...
            print(f"Error processing frame {database_id}, {f}:\n{e}")
            continue

    best_frames = sorted(frame_data, key=lambda x: x["pseudo_angle"])[:3]

    for i, bf in enumerate(best_frames):
        file_base_name = f"{database_id}_{i}"

        shutil.copyfile(bf["frame_path"], os.path.join(img_folder, f"{file_base_name}.jpg"))

        with open(os.path.join(txt_folder, f"{file_base_name}.txt"), "w") as f:
